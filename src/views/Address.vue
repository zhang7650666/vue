        ...
        <div class="checkout-page">
            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="addr-list">
                        <ul>
                            <li v-for="(item, index) in addressFilter" :class="{'check':checkedId == item.addressId}" @click="getCheckedId(item.addressId)">
                                <dl>
                                    <dt>{{item.userName}}</dt>
                                ...
                                <a href="javascript:;" class="addr-del-btn" @click="addrssDel(item)">
                    ...
                    <div class="shipping-addr-more">
                        <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showAll = !showAll" :class="{'open':showAll}">
                        more
                        <i class="i-up-down">
...
export default {
    data(){
        return{
            addressData:[],//当前用户的所欲地址
            limit:3,//显示默认条数
            showAll:false,//显示所有数据,
            checkedId:"",//默认选中的id
            del_pop:false,//modal弹框显示
            curGoodsId:"",//当前商品Id
        }
    },
    computed:{
        addressFilter(){
            if(this.showAll){
                return this.addressData.slice(0, this.limit);
            }else{
    ...
    methods:{
        ...
        //删除地址
        addrssDel(item){
            this.curGoodsId = item.addressId;
            this.del_pop = true;
        },
        //确认删除地址
        sure_del(){
